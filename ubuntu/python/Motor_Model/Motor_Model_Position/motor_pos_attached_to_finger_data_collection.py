...
def collect_data(dt, packet_handler, port_handler, scaled_voltage, scaled_pwm, addr_pro_goal_pwm, addr_present_position):
    ...
    try:
        for voltage, pwm in zip(scaled_voltage, scaled_pwm):
            ...
            data.append([voltage, dxl_present_position])
            print(f"Voltage: {voltage} Position: {dxl_present_position}")
            time.sleep(dt)
    finally:
        return data
...
def save_data_to_csv(data, filename):
    with open(filename, 'w', newline='') as file:
        ...
        writer.writerow(["Voltage", "Position"])
...
def main():
    ...
    try:
        ...
        save_data_to_csv(data, './Motor_Model/Motor_Model_Position/motor_data_pos.csv')
