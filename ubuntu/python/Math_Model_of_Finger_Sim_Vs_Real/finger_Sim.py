...
r1 = 7.9 # radius of rotation of joint 1 (Metacropophalagenal joint)
r2 = 7.8 # radius of rotation of joint 2 (Proximal joint)
r3 = 6.1 # radius of rotation of join 3 (Distal joint)
max_D_joint_angle = 72 
...
dt = 0.001  # should be small to ensure my model is accurate. At max 0.01
...
def create_visual_model(pos_P_joint_X, pos_P_joint_Y, Pos_D_joint_X, Pos_D_joint_Y, pos_tip_X, pos_tip_Y):
    ...
    finger_tip_mid = sphere(pos= vector(pos_tip_X, pos_tip_Y, 0), radius=0.6, color=color.white, make_trail = True)
...
def plot_joint_pos(motor_angles, pos_P_joint_Xs, Pos_D_joint_Xs, pos_tip_Xs, pos_P_joint_Ys, Pos_D_joint_Ys, pos_tip_Ys):
    ...
    '''Plots finger pos in 2D'''
def plot_joint_trajectory(motor_angles, pos_P_joint_Xs, Pos_D_joint_Xs, pos_tip_Xs, pos_P_joint_Ys, Pos_D_joint_Ys, pos_tip_Ys):
    plt.figure(figsize=(8, 6))
    
    # Plotting trajectories for each joint
    plt.plot(pos_P_joint_Xs, pos_P_joint_Ys, label='Proximal Joint', marker='o')
    plt.plot(Pos_D_joint_Xs, Pos_D_joint_Ys, label='Distal Joint', marker='^')
    plt.plot(pos_tip_Xs, pos_tip_Ys, label='Finger Tip', marker='x')
    # plt.plot(pos_tip_Xs, pos_tip_Ys, label='Tip')
    
    plt.xlabel('Position X (cm)')
    plt.ylabel('Position Y (cm)')
    plt.title('Joint Trajectories as a Function of Motor Angle')
    plt.legend()
    plt.grid(True)
...
def save_data(times,theta_ms,theta_M_joints, theta_P_joints, theta_D_joints, pwms):
    ...
    with open('./step_input_sim.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["time","theta_m", "theta_D_joint", "theta_P_joint" ,"theta_M_joint", "pwm"])
        writer.writerows(data)
    print("data saved at ./step_input_sim.csv")
...
async def main():
    try:
        ...
        times, pwms = create_step_input(total_time = 0.495, dt = 0.001, step_magnitude = 6, max_pwm = 855)
        sim_times = []
        # print("voltages: ", voltages)
        #simulate finger
        ...
        for pwm in pwms: 
            ...
            theta_m = math.radians(calculate_theta_m(prev_theta_m,voltage = 12,dt = dt)) 
        ...
        #Plot joint angles
        # plot_joint_angles(motor_positions, theta_M_joints, theta_P_joints, theta_D_joints)

        # plot_joint_pos(motor_positions, pos_P_joint_Xs, Pos_D_joint_Xs, pos_tip_Xs, pos_P_joint_Ys, Pos_D_joint_Ys, pos_tip_Ys)

        # plot_joint_trajectory(motor_positions, pos_P_joint_Xs, Pos_D_joint_Xs, pos_tip_Xs, pos_P_joint_Ys, Pos_D_joint_Ys, pos_tip_Ys)
        #Save data
