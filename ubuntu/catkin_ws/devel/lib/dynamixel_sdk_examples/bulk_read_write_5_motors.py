...
#This function converts unsigned 32 bit integers to signed
...
index = 0
dxl_goal_position_motor1 = [DXL_MINIMUM_POSITION_VALUE, DXL_MAXIMUM_POSITION_VALUE] # Goal position motor 1
dxl_goal_position_motor2 = [DXL_MINIMUM_POSITION_VALUE, DXL_MAXIMUM_POSITION_VALUE] # Goal position motor 2
dxl_goal_position_motor3 = [DXL_MINIMUM_POSITION_VALUE, DXL_MAXIMUM_POSITION_VALUE] # Goal position motor 3
dxl_goal_position_motor4 = [DXL_MINIMUM_POSITION_VALUE, DXL_MAXIMUM_POSITION_VALUE] # Goal position motor 4
dxl_goal_position_motor5 = [DXL_MINIMUM_POSITION_VALUE, DXL_MAXIMUM_POSITION_VALUE] # Goal position motor 5
...
while 1:
    ...
    # Allocate goal position value into byte array for Motor 1
    param_goal_position1 = [DXL_LOBYTE(DXL_LOWORD(dxl_goal_position_motor1[index])), DXL_HIBYTE(DXL_LOWORD(dxl_goal_position_motor1[index])), DXL_LOBYTE(DXL_HIWORD(dxl_goal_position_motor1[index])), DXL_HIBYTE(DXL_HIWORD(dxl_goal_position_motor1[index]))]

    # Allocate goal position value into byte array for Motor 2
    param_goal_position2 = [DXL_LOBYTE(DXL_LOWORD(dxl_goal_position_motor2[index])), DXL_HIBYTE(DXL_LOWORD(dxl_goal_position_motor2[index])), DXL_LOBYTE(DXL_HIWORD(dxl_goal_position_motor2[index])), DXL_HIBYTE(DXL_HIWORD(dxl_goal_position_motor2[index]))]

    # Allocate goal position value into byte array for Motor 3
    param_goal_position3 = [DXL_LOBYTE(DXL_LOWORD(dxl_goal_position_motor3[index])), DXL_HIBYTE(DXL_LOWORD(dxl_goal_position_motor3[index])), DXL_LOBYTE(DXL_HIWORD(dxl_goal_position_motor3[index])), DXL_HIBYTE(DXL_HIWORD(dxl_goal_position_motor3[index]))]

    # Allocate goal position value into byte array for Motor 4
    param_goal_position4 = [DXL_LOBYTE(DXL_LOWORD(dxl_goal_position_motor4[index])), DXL_HIBYTE(DXL_LOWORD(dxl_goal_position_motor4[index])), DXL_LOBYTE(DXL_HIWORD(dxl_goal_position_motor4[index])), DXL_HIBYTE(DXL_HIWORD(dxl_goal_position_motor4[index]))]

    # Allocate goal position value into byte array for Motor 5
    param_goal_position5 = [DXL_LOBYTE(DXL_LOWORD(dxl_goal_position_motor5[index])), DXL_HIBYTE(DXL_LOWORD(dxl_goal_position_motor5[index])), DXL_LOBYTE(DXL_HIWORD(dxl_goal_position_motor5[index])), DXL_HIBYTE(DXL_HIWORD(dxl_goal_position_motor5[index]))]


    # # Allocate goal position value into byte array for Motor 2
    # param_goal_position2 = [DXL_LOBYTE(DXL_LOWORD(dxl_goal_position_motor2[index])), 
    #                     DXL_HIBYTE(DXL_LOWORD(dxl_goal_position_motor2[index])), 
    #                     DXL_LOBYTE(DXL_HIWORD(dxl_goal_position_motor2[index])), 
    #                     DXL_HIBYTE(DXL_HIWORD(dxl_goal_position_motor2[index]))]
    

    
    # Add Dynamixel#1 goal position to the Bulkwrite parameter storage
    dxl_addparam_result = groupBulkWrite.addParam(DXL1_ID, ADDR_GOAL_POSITION, LEN_GOAL_POSITION, param_goal_position1)
    if dxl_addparam_result != True:
        print("[ID:%03d] groupBulkWrite addparam failed" % DXL1_ID)
        quit()

    # Add Dynamixel#2 goal position to the Bulkwrite parameter storage
    dxl_addparam_result = groupBulkWrite.addParam(DXL2_ID, ADDR_GOAL_POSITION, LEN_GOAL_POSITION, param_goal_position2)
    if dxl_addparam_result != True:
        print("[ID:%03d] groupBulkWrite addparam failed" % DXL2_ID)
        quit()

    # Add Dynamixel#3 goal position to the Bulkwrite parameter storage
    dxl_addparam_result = groupBulkWrite.addParam(DXL3_ID, ADDR_GOAL_POSITION, LEN_GOAL_POSITION, param_goal_position2)
    if dxl_addparam_result != True:
        print("[ID:%03d] groupBulkWrite addparam failed" % DXL3_ID)
        quit()

    # Add Dynamixel#4 goal position to the Bulkwrite parameter storage
    dxl_addparam_result = groupBulkWrite.addParam(DXL4_ID, ADDR_GOAL_POSITION, LEN_GOAL_POSITION, param_goal_position2)
    if dxl_addparam_result != True:
        print("[ID:%03d] groupBulkWrite addparam failed" % DXL4_ID)
        quit()

    # Add Dynamixel#5 goal position to the Bulkwrite parameter storage
    dxl_addparam_result = groupBulkWrite.addParam(DXL5_ID, ADDR_GOAL_POSITION, LEN_GOAL_POSITION, param_goal_position2)
    if dxl_addparam_result != True:
    ...
    while 1:
        ...
        #Convert unsigned to signed integers to read negative values correctly
        dxl1_present_position = convert_to_signed_32bit(dxl1_present_position)
        dxl2_present_position = convert_to_signed_32bit(dxl2_present_position)
        dxl3_present_position = convert_to_signed_32bit(dxl3_present_position)
        dxl4_present_position = convert_to_signed_32bit(dxl4_present_position)
        dxl5_present_position = convert_to_signed_32bit(dxl5_present_position)

       
        print("[ID:%03d] Present Position 1: %d \t [ID:%03d] Present Position 2: %d \t [ID:%03d] Present Position 3: %d \t [ID:%03d] Present Position 4: %d \t [ID:%03d] Present Position 5: %d" % 
        ...
        # Check if both motors reached their goal positions
        if not (abs(dxl_goal_position_motor1[index] - dxl1_present_position) > DXL_MOVING_STATUS_THRESHOLD) and \
            not (abs(dxl_goal_position_motor2[index] - dxl2_present_position) > DXL_MOVING_STATUS_THRESHOLD) and \
            not (abs(dxl_goal_position_motor3[index] - dxl3_present_position) > DXL_MOVING_STATUS_THRESHOLD) and \
            not (abs(dxl_goal_position_motor4[index] - dxl4_present_position) > DXL_MOVING_STATUS_THRESHOLD) and \
            not (abs(dxl_goal_position_motor5[index] - dxl5_present_position) > DXL_MOVING_STATUS_THRESHOLD):
                break
