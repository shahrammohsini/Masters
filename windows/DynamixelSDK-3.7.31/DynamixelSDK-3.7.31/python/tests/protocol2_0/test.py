...
goal_velocity = 100  # desired velocity if we're driving by velocity
dxl_step_from = dxl_goal_position[0]
dxl_step_to = dxl_goal_position[1]
...
step_size = abs(round((dxl_step_to-dxl_step_from)/4))
while 1:
    print("Press any key to continue! (or press ESC to quit!)")
    if getch() == chr(0x1b):
        break

    dxl_steps = range(dxl_step_from, dxl_step_to, step_size)

    for dxl_step in dxl_steps:
        print(dxl_step)
        # Write goal position
        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, DXL_ID, ADDR_PRO_GOAL_POSITION, dxl_step)
        if dxl_comm_result != COMM_SUCCESS:
            # time.sleep(0.5)
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
        ...
        while 1:
            ...
            print("goal: ", dxl_step, "current_pos: ", dxl_present_position)
            #print velocity
            # print("present velocity: ", dxl_present_velocity_signed)
            #print torque
            # print("present torque: ", dxl_present_load_signed)

            if not abs(dxl_step - dxl_present_position) > DXL_MOVING_STATUS_THRESHOLD:
                break
        ...
        # Reverse direction
    if dxl_step_from == DXL_MINIMUM_POSITION_VALUE:
        dxl_step_from = DXL_MAXIMUM_POSITION_VALUE
        dxl_step_to = DXL_MINIMUM_POSITION_VALUE
        step_size = -step_size
        print("if")
    else:
        dxl_step_from = DXL_MINIMUM_POSITION_VALUE
        dxl_step_to = DXL_MAXIMUM_POSITION_VALUE
