...
#Controller
error = 0
kp = 165.53 #(This is the ml model: 165.53)   150     # 1.3  300  150
ki = 32.77 # (This is the ml model: 32.77)    50      #eleminates steady state error  0.2   80
kd = 1.665 #  (This is the ml model: 1.665)    2         #prevent overshooting   0.2   5
previous_integral = 0
previous_error = 0
stall_torque = 1.1 # Nm
...
while t < 20:
    ...
    #Introduce a distrubance at t = 4.5 sec
    if (t >= 5 and t <= 5.1 ):
        Disturbance = 0.01
    else:
        Disturbance = 0

    # Mean squared error of angle in degrees
    Mean_squared_error = Mean_squared_error + (math.degrees(thetaR) - 0.1)**2 # Manual: 4366.8    ML: 2541.94
    
    
    #Simulation
    #thetaddotR = ((0.5*mR + mF)*g*L*thetaR - Torque_M) / ((1/3)*mR*L*L + mF*L*L)  #Linear equation
    #thetaddotR = ((M*g*L*sin(thetaR)) - Torque_M) / IL
    thetaddotR = ((0.5*mR + mF)*g*L*sin(thetaR) - Torque_M + Disturbance) / ((1/3)*mR*L*L + mF*L*L)  #without friction
...
Mean_squared_error = Mean_squared_error / i
