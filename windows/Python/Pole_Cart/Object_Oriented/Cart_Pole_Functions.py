###########################This file holds all the functions for a cart pole visual simulation###################
########################### Shahram Mohsini ###################
from vpython import *
import math
#Parameters
...
class Visual_Model:
    ...
    def Visual_Modle_Make(self):
        ...
        self.cart = box(color = color.blue, pos = vector(Xc,0.52,0.5), size = vector(3,1,3))
    ...
    def run_visual_sim(self,run_time = 10):
        ...
        Xcdot = 0 #initial velocity of cart
        thetadot = 0 # inital angular velocity
        vToF_conversionFactor = 1.72 #N·V-1
        ...
        while t < run_time:
            ...
            Force = self.voltage*vToF_conversionFactor
            Xcddot = (Force + self.m*g*sin(self.theta)*cos(self.theta) - self.m*self.L*thetadot*thetadot*sin(self.theta))/(self.M + self.m + self.m*cos(self.theta)*cos(self.theta))  #Non-Linear
            ...
            thetaddot = (Force*cos(self.theta) + (self.M + self.m)*g*sin(self.theta) - self.m*self.L*thetadot*thetadot*sin(self.theta)*cos(self.theta))/(self.L*(self.M + self.m) - self.m*self.L*cos(self.theta)*cos(self.theta)) #Non-Linear
    ...
    def graph_model(self):
        ...
        self.Graph_P_Pos = graph(title = 'Pendulum Position vs Time', width = 400, height = 200, xtitle = "Time [s]", ytitle = "Angle [deg]", fast = False)
        self.f_Pendulum_Pos = gcurve(color=color.blue)
        # graph of pendulum velocity
        self.Graph_P_V = graph(title = "Pendulum Velocity vs Time", width = 400, height = 200, xtitle = "Time [s]", ytitle = "Angular velocity [deg/s]", fast = False)
        self.f_Pendulum_Vel = gcurve(color = color.red)
        #graph of pendulum accelaration
        self.Graph_P_A = graph(title = 'Pendulum Accelaration vs Time', width = 400, height = 200, xtitle = "Time [s]", ytitle = "Angular Accelaration [deg/s^2]", fast = False)
        self.f_Pendulum_A = gcurve(color=color.orange)
        # graph of cart position
        self.Graph_C_P = graph(title = "Cart Position vs Time", width = 400, height = 200, xtitle = "Time [s]", ytitle = "Position [m]", fast = False)
        self.f_Cart_Pos = gcurve(color = color.purple)
        # graph of cart velocity
        self.Graph_C_V = graph(title = "Cart Velocity vs Time", width = 400, height = 200, xtitle = "Time [s]", ytitle = "Velocity [m/s]", fast = False)
