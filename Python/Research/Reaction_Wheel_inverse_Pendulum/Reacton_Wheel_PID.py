...
#Parameters
thetaR = 90 # angle of rod
thetaR = -math.radians(thetaR)
print(thetaR)
radius_f = 0.05
g = 9.8
L = 0.17 # Length of Rod
...
error = 0
kp = 1.3
kd = 0.2 #prevent overshooting   0.5
ki = 0.008  #eleminates steady state error
previous_integral = 0
previous_error = 0
stall_torque =  1.1
...
#Graphs
#graph of Rod angle
Graph_Motor_Torque = graph(title = 'Motor_Torque vs Time', width = 400, height = 200, xtitle = "Time [s]", ytitle = "Torque", fast = False)
f_Motor_Torque = gcurve(color=color.blue)
#graph of Rod angle
Graph_Rod_Angle = graph(title = 'Rod Angle vs Time', width = 400, height = 200, xtitle = "Time [s]", ytitle = "Angle [deg]", fast = False)
f_Rod_Angle = gcurve(color=color.blue)
#graph of Rod Velocity
Graph_Rod_velocity = graph(title = 'Rod Velocity vs Time', width = 400, height = 200, xtitle = "Time [s]", ytitle = "Angular Velcity [deg/s]", fast = False)
f_Rod_velocity = gcurve(color=color.blue)
#graph of Rod Accelaration
Graph_Rod_accelaration = graph(title = 'Rod Accelaration vs Time', width = 400, height = 200, xtitle = "Time [s]", ytitle = "Angular Accelaration [deg/s^2]", fast = False)
...
Graph_Flywheel_velocity = graph(title = 'Flywheel Velocity vs Time', width = 400, height = 200, xtitle = "Time [s]", ytitle = "Angular Velcity [deg/s]", fast = False)
...
while t < 120:
    ...
    thetaddotR = ((0.5*mR + mF)*g*L*sin(thetaR) - Torque_M) / ((1/3)*mR*L*L + mF*L*L)
    ...
    fly_Wheel.rotate(angle = (thetadotw/60), axis=vector(0, 0, 1))
    ...
    f_Flywheel_velocity.plot(t,-math.degrees(thetadotw)) #Plot thetadotf in deg/s
